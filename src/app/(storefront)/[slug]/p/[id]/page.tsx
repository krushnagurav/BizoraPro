...
export default async function ProductDetailPage({
  ...
  const { data: product } = await supabase
    .from("products")
    .select(
      "*, shops(id, name, theme_config, is_open, auto_close, opening_time, closing_time), categories(name)",
    )
    .eq("id", id)
    .single();
  ...
  if (product.shops.auto_close) {
    ...
    if (
      currentHM < (product.shops.opening_time || "09:00") ||
      currentHM > (product.shops.closing_time || "21:00")
    ) {
            ...
            {product.gallery_images?.length > 0 && (
              <div className="grid grid-cols-4 gap-4">
                ...
                {product.gallery_images.map((img: string, i: number) => (
                  <div
                    key={i}
                    className="aspect-square relative rounded-lg overflow-hidden border border-transparent hover:border-primary/50 cursor-pointer bg-secondary/10"
                  >
                    <Image
                      src={img}
                      fill
            ...
            <div className="flex items-end gap-4 mb-8">
              ...
              {product.sale_price && (
                <>
                  ...
                  <Badge className="bg-red-100 text-red-600 hover:bg-red-100 mb-2">
