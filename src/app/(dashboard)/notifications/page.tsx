    ...
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Updates, announcements, and alerts.</p>
        </div>
      ...
      <div className="space-y-4">
        {notifications?.length === 0 ? (
           <div className="text-center py-20 bg-secondary/10 rounded-xl border border-dashed border-border/50">
              ...
              <p className="text-sm text-muted-foreground/50">You have no new notifications.</p>
           </div>
        ) : (
          notifications?.map((n) => (
            <Card key={n.id} className={`border-border/50 transition-all ${!n.is_read ? 'bg-secondary/20 border-primary/30' : 'bg-card opacity-80'}`}>
              <CardContent className="p-4 flex gap-4 items-start">
                 ...
                 <div className="flex-1 space-y-1">
                    ...
                    {n.type === 'announcement' && (
                       <Badge variant="secondary" className="mt-2 text-[10px] bg-blue-500/10 text-blue-400 hover:bg-blue-500/20">New Feature</Badge>
                    )}
                 </div>
                 {!n.is_read && (
