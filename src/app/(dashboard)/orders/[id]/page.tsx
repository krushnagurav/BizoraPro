        ...
        <div className="flex items-center gap-4">
          ...
          <div>
             <h1 className="text-2xl font-bold flex items-center gap-2">
               ...
               <Badge variant={order.status === 'delivered' ? 'secondary' : 'outline'} className="capitalize">
                 {order.status}
               </Badge>
             </h1>
         ...
         <div className="md:col-span-2 space-y-6">
            <Card className="bg-card border-border/50">
              ...
              <CardContent>
                 <div className="space-y-4">
                   {items.map((item, i) => (
                     <div key={i} className="flex justify-between items-center border-b border-border/50 pb-4 last:pb-0 last:border-0">
                        <div className="flex gap-4">
                           {/* Optional: Image if you saved it in items JSON */}
                           <div>
                              <p className="font-medium">{item.name}</p>
                              <p className="text-xs text-muted-foreground">Qty: {item.qty} x ₹{item.price}</p>
                           </div>
                        </div>
            ...
            {isFinal && (
              <div className="text-center p-6 bg-secondary/10 rounded-xl text-muted-foreground border border-border/50">
                 Order is <strong>{order.status}</strong>. No further actions allowed.
              </div>
            )}
         </div>
               ...
               <CardContent className="space-y-6">
                  ...
                  <div className="flex gap-3 items-start">
                     ...
                     <div>
                        ...
                        <a href={`https://wa.me/${customer.phone}`} target="_blank" className="text-xs text-green-500 hover:underline">Open Chat</a>
                     </div>
                  </div>
                  ...
                  <div className="flex gap-3 items-start">
                     ...
                     <div>
                        ...
                        <p className="font-medium">{customer.address || "No street address"}</p>
