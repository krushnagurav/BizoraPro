...
import { Loader2, ShieldCheck, RotateCcw, Lock, Wand2, Eye } from "lucide-react";
...
export default function PoliciesPage() {
  ...
  useEffect(() => {
    const fetchShop = async () => {
      ...
      if (user) {
        const { data } = await supabase.from("shops").select("name, slug, policies").eq("owner_id", user.id).single();
        setShop(data);
        if (data?.policies) {
    ...
    <div className="p-8 max-w-4xl space-y-8">
      <div className="flex items-center justify-between">
        <div>
           ...
           <p className="text-muted-foreground">Define rules to build trust with your customers.</p>
        </div>
        <Link href={`/${shop.slug}/legal`} target="_blank">
           <Button variant="outline" className="gap-2"><Eye className="h-4 w-4" /> View Live Page</Button>
        </Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Tabs defaultValue="refund" className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-8 bg-card border border-border/50">
            <TabsTrigger value="refund" className="gap-2"><RotateCcw className="h-4 w-4"/> Refund Policy</TabsTrigger>
            <TabsTrigger value="privacy" className="gap-2"><Lock className="h-4 w-4"/> Privacy Policy</TabsTrigger>
            <TabsTrigger value="terms" className="gap-2"><ShieldCheck className="h-4 w-4"/> Terms of Service</TabsTrigger>
          </TabsList>
             ...
             <Card className="bg-card border-border/50">
                <CardHeader className="flex flex-row items-center justify-between">
                   <div>
                      ...
                      <CardDescription>Explain how customers can return items.</CardDescription>
                   </div>
                   <Button type="button" variant="secondary" size="sm" onClick={() => handleGenerate('refund')} className="gap-2 text-xs">
          ...
          <TabsContent value="privacy">
             <Card className="bg-card border-border/50">
                <CardHeader className="flex flex-row items-center justify-between">
                   ...
                   <Button type="button" variant="secondary" size="sm" onClick={() => handleGenerate('privacy')} className="gap-2 text-xs">
                      <Wand2 className="h-3 w-3" /> Use Template
                   </Button>
                </CardHeader>
                <CardContent>
          ...
          <TabsContent value="terms">
             <Card className="bg-card border-border/50">
                <CardHeader className="flex flex-row items-center justify-between">
                   ...
                   <Button type="button" variant="secondary" size="sm" onClick={() => handleGenerate('terms')} className="gap-2 text-xs">
                      <Wand2 className="h-3 w-3" /> Use Template
                   </Button>
                </CardHeader>
                <CardContent>
          ...
          <div className="mt-6 flex justify-end">
             <Button size="lg" className="font-bold bg-primary text-black hover:bg-primary/90" disabled={loading}>
