...
export function ImpersonateButton({ userId }: { userId: string }) {
  ...
  const handleImpersonate = async () => {
    const confirm = window.confirm("Are you sure? You will be logged out of Admin and logged in as this user.");
    if (!confirm) return;
  ...
  return (
    ...
    >
      {loading ? <Loader2 className="animate-spin h-4 w-4"/> : <UserCog className="h-4 w-4" />}
      Impersonate
    </Button>
  );
}