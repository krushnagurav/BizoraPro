      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <div className="flex gap-2">
           <Select defaultValue="all">
              <SelectTrigger className="h-8 w-[120px] bg-background"><SelectValue placeholder="Status" /></SelectTrigger>
              <SelectContent>
              ...
              invoices.map((inv) => (
                <TableRow key={inv.id} className="border-border hover:bg-secondary/10">
                  ...
                  <TableCell>
                    <Badge 
                      className={
                        inv.status === 'succeeded' ? "bg-green-500/10 text-green-500 hover:bg-green-500/20" : 
                        inv.status === 'pending' ? "bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20" :
