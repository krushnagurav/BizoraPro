...
import { AlertCircle, ArrowRight, CheckCircle2, Clock, Headphones } from "lucide-react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         <Card className="bg-[#111] border-white/10 text-white">
            <CardContent className="p-4 flex items-center justify-between">
               <div><p className="text-xs text-gray-400 uppercase">Open Tickets</p><p className="text-2xl font-bold text-blue-400">{open.length}</p></div>
               <Clock className="h-6 w-6 text-blue-400 opacity-20" />
            </CardContent>
         </Card>
         <Card className="bg-[#111] border-white/10 text-white">
            <CardContent className="p-4 flex items-center justify-between">
               <div><p className="text-xs text-gray-400 uppercase">Critical / High</p><p className="text-2xl font-bold text-red-500">{critical}</p></div>
               <AlertCircle className="h-6 w-6 text-red-500 opacity-20" />
            </CardContent>
         </Card>
         <Card className="bg-[#111] border-white/10 text-white">
            <CardContent className="p-4 flex items-center justify-between">
               <div><p className="text-xs text-gray-400 uppercase">Resolved</p><p className="text-2xl font-bold text-green-500">{resolved.length}</p></div>
         ...
         <Card className="bg-[#111] border-white/10 text-white">
            <CardContent className="p-4 flex items-center justify-between">
               <div><p className="text-xs text-gray-400 uppercase">Total Volume</p><p className="text-2xl font-bold">{total}</p></div>
...
function TicketTable({ tickets }: { tickets: any[] }) {
   const priorityColors: any = { low: "text-blue-400 border-blue-400/20", medium: "text-yellow-400 border-yellow-400/20", high: "text-orange-500 border-orange-500/20", critical: "text-red-500 border-red-500/20 bg-red-500/10" };
        ...
        <CardContent className="p-0">
           <Table>
             ...
             <TableBody>
               {tickets.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="h-32 text-center text-gray-500">No tickets found.</TableCell>
                  </TableRow>
               ) : (
                 tickets.map((t) => (
                   <TableRow key={t.id} className="border-white/10 hover:bg-white/5">
                     ...
                     <TableCell>
                        ...
                        <Badge variant="outline" className="text-[10px] border-white/20 text-gray-400">{t.shops?.plan}</Badge>
                     </TableCell>
                     <TableCell>
                        <Badge variant="outline" className={`capitalize ${priorityColors[t.priority]}`}>
                           {t.priority}
                        </Badge>
                     </TableCell>
