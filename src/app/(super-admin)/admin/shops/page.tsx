              ...
              {shops?.map((shop) => (
                <TableRow key={shop.id} className="border-white/10 hover:bg-white/5">
                  ...
                  <TableCell className="text-blue-400">
                    <a href={`http://localhost:3000/${shop.slug}`} target="_blank">/{shop.slug}</a>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className="border-primary text-primary capitalize">{shop.plan}</Badge>
                  </TableCell>
                  <TableCell>
                    <Badge className={shop.is_open ? "bg-green-900 text-green-300" : "bg-red-900 text-red-300"}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      {/* Visit Button */}
                      <a href={`http://localhost:3000/${shop.slug}`} target="_blank">
                      ...
                      <form action={toggleShopStatusAction}>
                        ...
                        <input type="hidden" name="currentStatus" value={String(!shop.is_open)} />
                        <Button size="icon" variant="ghost" className="text-red-500 hover:text-red-400 hover:bg-red-900/20">
