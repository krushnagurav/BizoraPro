...
export function OrderTimeline({ status }: { status: string }) {
  ...
  if (isCancelled) {
     return <div className="text-red-500 font-bold border p-4 rounded-lg border-red-500/50 bg-red-500/10">Order Cancelled</div>;
  }
        ...
        return (
          <div key={step.id} className="flex flex-col items-center bg-background px-2">
            <div className={cn(
              ...
              isActive ? "bg-primary border-primary text-black" : "bg-secondary border-secondary text-muted-foreground"
            )}>
