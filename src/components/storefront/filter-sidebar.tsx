...
export function FilterSidebar({ slug }: { slug: string }) {
  ...
  const applyFilters = () => {
    ...
    if (inStock) params.set("stock", "true"); else params.delete("stock");
    if (rating) params.set("rating", rating.toString()); else params.delete("rating");
    ...
    <div className="w-64 shrink-0 space-y-8 hidden lg:block">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Label htmlFor="stock" className="text-slate-600 font-normal cursor-pointer">In Stock Only</Label>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {[5, 4, 3].map((stars) => (
          <div key={stars} className="flex items-center gap-2 cursor-pointer" onClick={() => setRating(stars)}>
            <Checkbox checked={rating === stars} onCheckedChange={() => setRating(stars)} id={`r-${stars}`} />
            <label htmlFor={`r-${stars}`} className="flex items-center gap-1 cursor-pointer text-sm text-slate-600">
      ...
      <Button onClick={applyFilters} className="w-full font-bold bg-slate-900 text-white">
        Apply Filters
      </Button>
