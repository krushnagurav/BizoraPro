...
import { ArrowLeft, Clock, Instagram, MessageCircle, Phone } from "lucide-react";
    ...
    >
      ...
      <div className="max-w-md mx-auto px-4 py-12 space-y-8">
         <div className="text-center space-y-2">
            <h1 className="text-3xl font-bold text-slate-900">Contact the Shop</h1>
            <p className="text-slate-500">Get in touch with us for any questions</p>
         </div>
         ...
         <Card className="bg-white border-0 shadow-xl shadow-slate-200/60 rounded-3xl overflow-hidden">
            <CardContent className="p-8 space-y-8 text-center">
               ...
               <div className="relative w-24 h-24 mx-auto rounded-full border-4 border-white shadow-lg overflow-hidden bg-slate-100">
                  {theme.logoUrl ? (
                    <Image src={theme.logoUrl} fill className="object-cover" alt="Logo" unoptimized />
                  ) : (
                    <div className="flex items-center justify-center h-full text-2xl font-bold text-slate-400">{shop.name.charAt(0)}</div>
                  )}
               </div>
               ...
               <a href={`https://wa.me/${shop.whatsapp_number}`} target="_blank" className="block w-full">
               ...
               <div className="space-y-4 w-full text-left">
                 <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                    <div className="flex items-center gap-4">
                       <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center"><Phone className="w-5 h-5"/></div>
                       <div><p className="font-bold text-slate-900">Phone</p><p className="text-sm text-slate-500">+91 {shop.whatsapp_number}</p></div>
                    </div>
                    <a href={`tel:${shop.whatsapp_number}`} className="text-sm font-bold text-blue-600">Call</a>
                 </div>

                 {social.instagram && (
                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                      <div className="flex items-center gap-4">
                         <div className="w-10 h-10 bg-pink-100 text-pink-600 rounded-full flex items-center justify-center"><Instagram className="w-5 h-5"/></div>
                         <div><p className="font-bold text-slate-900">Instagram</p><p className="text-sm text-slate-500">@{social.instagram.split('/').pop()}</p></div>
                      </div>
                      <a href={social.instagram} target="_blank" className="text-sm font-bold text-pink-600">Follow</a>
                    </div>
                 )}
               </div>
            </CardContent>
         ...
         <div className="bg-blue-50/50 rounded-3xl p-8 text-center space-y-2 border border-blue-100">
            ...
            <p className="text-sm text-slate-500">
               {shop.opening_time ? `${shop.opening_time} - ${shop.closing_time}` : "9:00 AM - 9:00 PM"}
            </p>
         </div>
