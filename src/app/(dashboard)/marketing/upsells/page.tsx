...
import { ArrowRight, Trash2, Plus, TrendingUp } from "lucide-react";
import Image from "next/image";
import { FeatureLock } from "@/src/components/dashboard/shared/feature-lock"; // Import Lock
...
export default async function UpsellPage() {
  ...
  // Fetch Data (Only if PRO to save resources, or fetch anyway to show blurred background)
  const { data: products } = await supabase.from("products").select("id, name").eq("shop_id", shop?.id).eq("status", "active");
  const { data: upsells } = await supabase.from("upsells").select("*, trigger:trigger_product_id(name, image_url), suggested:suggested_product_id(name, image_url)").eq("shop_id", shop?.id);
    ...
    <div className="p-8 space-y-8 max-w-4xl mx-auto">
      ...
      <FeatureLock plan={shop?.plan} featureName="Upsell Recommendations">
        ...
        <Card className="bg-card border-border/50 mb-8">
          <CardHeader><CardTitle>Add Recommendation</CardTitle></CardHeader>
          <CardContent>
            <form action={createUpsellAction} className="flex flex-col md:flex-row gap-4 items-end">
              ...
              <Button type="submit" className="font-bold"><Plus className="h-4 w-4 mr-2" /> Add</Button>
            </form>
          </CardContent>
        </Card>
            ...
            <Card key={item.id} className="bg-secondary/10 border-border/50">
              <CardContent className="p-4 flex items-center justify-between">
                ...
                <form action={deleteUpsellAction}>
                  ...
                  <Button size="icon" variant="ghost" className="text-red-500 hover:bg-red-900/10"><Trash2 className="h-4 w-4" /></Button>
                </form>
              </CardContent>
            </Card>
          ))}
