      ...
      <div className="aspect-square relative bg-slate-100 overflow-hidden">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          ...
          {!isOutOfStock && discountPercentage > 0 && (
            <Badge className="bg-red-600 text-[10px] px-2">
              {discountPercentage}% OFF
            </Badge>
          )}
          {!isOutOfStock && product.badges?.includes("bestseller") && (
            <Badge className="bg-yellow-500 text-black text-[10px] px-2">
              Bestseller
            </Badge>
          )}
          {!isOutOfStock && product.badges?.includes("new") && (
            <Badge className="bg-blue-600 text-[10px] px-2">New</Badge>
          )}
        </div>
      </div>
      ...
      <CardContent className="p-4 flex flex-col flex-1">
        ...
        {hasVariants ? (
          <Link href={`/product/${product.id}`} className="w-full">
            <Button
              variant="outline"
              className="w-full font-bold border-primary/50 text-primary hover:bg-primary hover:text-black transition-colors"
            >
              Select Options
            </Button>
          </Link>
