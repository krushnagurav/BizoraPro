...
export default async function SearchPage({ 
  ...
  searchParams: Promise<{ q?: string; min?: string; max?: string; stock?: string }>
}) {
  const { slug } = await params;
  const { q, min, max, stock } = await searchParams;
  ...
  // Note: 'stock' filter logic depends on if you track inventory count. 
  // For MVP we assume active status is enough, or use stock_count > 0 logic if you have it.
  ...
  if (shop.auto_close) {
    ...
    if (currentHM < (shop.opening_time || "09:00") || currentHM > (shop.closing_time || "21:00")) {
      isShopActuallyOpen = false;
    }
  }

  return (
    <div className="min-h-screen bg-[#F8F9FA]" style={{ "--primary": primaryColorHsl } as React.CSSProperties}>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        ...
        <div className="mb-8 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
           <h1 className="text-2xl font-bold text-slate-900 mb-4">
             {products?.length} results found {q && `for "${q}"`}
           </h1>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                {products.map((product) => (
                  <Link href={`/${slug}/p/${product.id}`} key={product.id}>
                    <ProductCard product={product} isShopOpen={isShopActuallyOpen} />
                  </Link>
                ))}
              </div>
            ) : (
