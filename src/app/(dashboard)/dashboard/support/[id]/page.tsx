      ...
      <div className="flex items-center justify-between mb-4 shrink-0">
         <div className="flex items-center gap-3">
            ...
            <div>
               ...
               <p className="text-xs text-muted-foreground">Ticket #{ticket.id.slice(0,8)} • {ticket.priority} priority</p>
            </div>
         </div>
         ...
         {ticket.status !== 'resolved' && (
            <form action={updateTicketStatusAction}>
               ...
               <Button variant="outline" size="sm" className="gap-2 text-green-500 hover:text-green-600 hover:bg-green-500/10">
               ...
               return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                     <div className={`max-w-[80%] p-4 rounded-2xl text-sm ${
                        ...
                        <p>{msg.message}</p>
                        <p className={`text-[10px] mt-2 text-right ${isMe ? 'text-black/50' : 'text-muted-foreground'}`}>
            ...
            {ticket.status === 'resolved' && (
               <div className="flex justify-center">
                  <div className="bg-green-500/10 text-green-500 px-4 py-2 rounded-lg text-sm flex items-center gap-2">
                     <CheckCircle2 className="h-4 w-4" /> This ticket has been marked as resolved.
                  </div>
               </div>
            )}
         </div>
