...
import { AlertTriangle, ArrowLeft, ArrowRight, Check, MessageSquare, Star, Trash2 } from "lucide-react";
import Image from "next/image";
import Link from "next/link";

export default async function ReviewsPage({ searchParams }: { searchParams: Promise<{ status?: string, stars?: string, page?: string }> }) {
  ...
  const { data: allReviews } = await supabase.from("product_reviews").select("rating, is_approved").eq("shop_id", shop?.id);
  ...
  const avgRating = total > 0 ? (allReviews?.reduce((acc, r) => acc + r.rating, 0) || 0) / total : 0;
  ...
  return (
    <div className="p-8 space-y-8 max-w-5xl mx-auto">
      ...
      <div className="grid gap-4">
                           ...
                           <div className="flex items-center gap-1 bg-secondary/30 px-1.5 py-0.5 rounded">
                                {review.products?.image_url && (
                                    <div className="relative w-3 h-3 rounded overflow-hidden">
                                        <Image src={review.products.image_url} fill className="object-cover" alt="" />
                                    </div>
                                )}
                                <span className="font-medium text-foreground">{review.products?.name || "Unknown Product"}</span>
                           </div>
              ...
              <div className="flex justify-end gap-2 border-t border-border/50 pt-4">
                ...
                {!review.is_approved && (
                  <form action={toggleReviewStatusAction}>
                    ...
                    <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white gap-2"><Check className="h-3 w-3" /> Approve</Button>
                  </form>
                )}
                <form action={toggleReviewStatusAction}>
                  <input type="hidden" name="id" value={review.id} />
                  <input type="hidden" name="action" value="delete" />
                  <Button size="sm" variant="ghost" className="text-red-500 hover:bg-red-900/10"><Trash2 className="h-4 w-4" /></Button>
                </form>
              </div>
        ...
        {reviews?.length === 0 && <div className="text-center py-12 text-muted-foreground">No reviews matching filters.</div>}
        
        {/* Pagination */}
        {totalPages > 1 && (
            <div className="flex justify-center gap-2 pt-4">
                <Link href={`/marketing/reviews?page=${page-1}&status=${statusFilter}&stars=${starsFilter}`}>
                    <Button variant="outline" size="sm" disabled={page <= 1}><ArrowLeft className="h-4 w-4" /> Prev</Button>
                </Link>
                <Link href={`/marketing/reviews?page=${page+1}&status=${statusFilter}&stars=${starsFilter}`}>
                    <Button variant="outline" size="sm" disabled={page >= totalPages}>Next <ArrowRight className="h-4 w-4" /></Button>
                </Link>
            </div>
        )}
      </div>
