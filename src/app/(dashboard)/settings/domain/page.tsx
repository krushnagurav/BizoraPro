    ...
    const fetchShop = async () => {
      ...
      if (user) {
        const { data } = await supabase
          .from("shops")
          .select("custom_domain, domain_verified, plan, slug")
          .eq("owner_id", user.id)
          .single();
        setShop(data);
    ...
    <div className="p-8 max-w-3xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Connect your own domain (e.g., mystore.com) to establish your brand.
        </p>
      </div>
        ...
        <Card className="bg-secondary/10 border-dashed border-yellow-500/50">
          <CardContent className="p-8 text-center flex flex-col items-center">
            ...
            <p className="text-muted-foreground max-w-md mb-6">
              Connect your own domain to build trust and look professional.
              Available exclusively on the <strong>Pro Plan</strong>.
            </p>
            <Link href="/billing">
          ...
          <CardHeader className="border-b border-border/50 pb-4">
            <div className="flex justify-between items-center">
              ...
              {shop.domain_verified ? (
                <Badge className="bg-green-500 text-white gap-1 hover:bg-green-600">
                  <CheckCircle2 className="w-3 h-3" /> Live
                </Badge>
              ) : (
                <Badge
                  variant="outline"
                  className="border-yellow-500 text-yellow-500 gap-1 animate-pulse"
                >
      ...
      {isPro && !shop.custom_domain && (
        <Card className="bg-card border-border/50">
          <CardHeader>
            ...
            <CardDescription>
              Enter the domain you want to use for your shop.
            </CardDescription>
          </CardHeader>
          <CardContent>
